...
"@
					...
					Write-Host " `nThere is some error while sending the email. Please send the email manually"     
...
$select_quota = Read-Host "Please Press 1 to set Disk Hard Quota to Default 2GB or Press 2 to enter new value:"
        switch($select_quota){
            ...
            2 {
                [int]$Value = Read-Host "Enter the value in Disk Space in GBs:"
                $global:hdquota = $Value * 1024 * 1024
                Write-Host "The total disk space allocated will be $Value GB ($global:hdquota KB).Press enter to continue"
                pause
              }
